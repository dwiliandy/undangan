...
@section('content')
	...
	<button id="music-control"
		class="fixed bottom-6 right-6 z-50 bg-yellow-600 text-white p-4 rounded-full shadow-2xl hidden hover:bg-yellow-700 transition"
		onclick="toggleMusic()">
		<i class="fas fa-compact-disc spin text-xl" id="music-icon"></i>
	</button>

	<!-- COVER SECTION -->
	<div id="cover" class="fixed inset-0 z-50 bg-yellow-50 flex flex-col justify-center items-center text-center p-6">
		<div class="mb-8 animate__animated animate__tada">
			@if (isset($event->birthdayEvent->photo))
				<div class="w-40 h-40 rounded-full border-4 border-yellow-300 overflow-hidden mx-auto shadow-lg">
		...
		<p class="text-yellow-600 uppercase tracking-widest mb-4">You Are Invited To</p>
		...
		@if (isset($event->birthdayEvent->age))
			<p class="text-2xl text-yellow-600 font-bold mb-8">Level {{ $event->birthdayEvent->age }} Unlocked!</p>
		@endif

		<!-- Countdown -->
		<div class="flex space-x-4 text-center mb-10">
			<div class="bg-white p-3 rounded shadow-sm w-20">
				<span id="c-days" class="text-2xl font-bold block text-yellow-600">00</span>
				<span class="text-xs uppercase text-gray-400">Days</span>
			</div>
			<div class="bg-white p-3 rounded shadow-sm w-20">
				<span id="c-hours" class="text-2xl font-bold block text-yellow-600">00</span>
				<span class="text-xs uppercase text-gray-400">Hrs</span>
			</div>
			<div class="bg-white p-3 rounded shadow-sm w-20">
				<span id="c-minutes" class="text-2xl font-bold block text-yellow-600">00</span>
				<span class="text-xs uppercase text-gray-400">Min</span>
			</div>
			<div class="bg-white p-3 rounded shadow-sm w-20">
				<span id="c-seconds" class="text-2xl font-bold block text-yellow-600">00</span>
		...
		<button id="open-invitation"
			class="bg-yellow-600 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-yellow-700 transition transform hover:scale-105 open-btn-animate">
			Open Invitation
		</button>
	</div>
	...
	<div id="main-content"
		class="hidden min-h-screen bg-yellow-100 flex flex-col items-center justify-center p-4 text-center pb-20">
		<div class="bg-white p-8 rounded-lg shadow-lg max-w-md w-full" data-aos="zoom-in-up">
			<h1 class="text-4xl font-bold text-yellow-600 mb-2">Shine Bright!</h1>
			<p class="text-gray-500 uppercase tracking-widest mb-6">Birthday Celebration</p>

			<div class="mb-6">
				@if (isset($event->birthdayEvent->photo))
					<div class="w-48 h-48 rounded-full border-4 border-yellow-100 overflow-hidden mx-auto shadow-md mb-4">
						<img src="{{ Storage::url($event->birthdayEvent->photo) }}" class="w-full h-full object-cover">
					</div>
				@else
					<div class="w-32 h-32 bg-yellow-50 rounded-full mx-auto flex items-center justify-center overflow-hidden mb-4">
						<span class="text-4xl">✨</span>
					</div>
				@endif
				<h2 class="text-3xl font-bold text-gray-800">{{ $event->birthdayEvent->person_name ?? 'Birthday Kid' }}</h2>
				@if (isset($event->birthdayEvent->age))
					<p class="text-xl text-yellow-500 font-bold">Turning {{ $event->birthdayEvent->age }}</p>
				@endif
			</div>
					...
					<div class="mt-4">
						@foreach ($event->eventLocations as $loc)
							<div class="mb-2">
								...
								<p class="text-sm font-bold text-yellow-500">{{ \Carbon\Carbon::parse($loc->event_time)->format('H:i') }}</p>
								@if ($loc->google_maps_url)
									<a href="{{ $loc->google_maps_url }}" target="_blank" class="text-xs text-yellow-500 underline">View Map</a>
								@endif
							</div>
						@endforeach
					</div>
			...
			<div class="mb-8">
				<p class="text-gray-600 mb-4">"Let's make some noise and celebrate!"</p>
			</div>

			<form id="rsvp-form" action="{{ route('frontend.rsvp', [$event->slug, $invitation->slug]) }}" method="POST"
				class="bg-yellow-50 p-4 rounded text-left">
				@csrf
				<h3 class="font-bold text-yellow-600 mb-3">RSVP</h3>
				<div class="mb-3">
				...
				<button type="submit"
					class="w-full bg-yellow-600 text-white font-bold py-2 rounded hover:bg-yellow-700 text-sm">Send</button>
			</form>
		</div>
