...
@section('content')
	...
	<div id="cover"
		class="fixed inset-0 z-50 flex flex-col justify-center items-center text-center animate-fade-in bg-[#141414] px-4 overflow-y-auto">
		<div class="m-auto w-full max-w-4xl py-10">
			...
			<div class="flex flex-wrap justify-center gap-6 md:gap-16 max-w-4xl">
				...
				<div class="group cursor-pointer open-invitation-trigger relative profile-item">
					<div
						...
						<!-- Edit Overlay -->
						<div
							class="absolute inset-0 bg-black/70 flex items-center justify-center opacity-0 transition edit-overlay hidden">
				...
				<section id="rsvp-section" class="max-w-4xl mx-auto pt-10">
					<div class="bg-black/50 p-8 rounded-lg border border-gray-800">
						...
						<form id="rsvp-form" action="{{ route('frontend.rsvp', [$event->slug, $invitation->slug]) }}" method="POST"
							...
							<button type="submit"
								class="w-full bg-[#E50914] hover:bg-red-700 text-white font-bold py-4 rounded transition">
								Submit Review
							</button>
						</form>
					</div>
			...
			triggers.forEach(trigger => {
				trigger.addEventListener('click', () => {
					...
					setTimeout(() => {
						...
						// Force a reflow or slight delay to ensure browser acknowledges visible state before AOS
						setTimeout(() => {
							AOS.init();
						}, 50);
					}, 500);
