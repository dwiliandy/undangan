	...
	<div id="cover"
		...
		<div class="relative mb-8">
			...
			<h1 class="relative text-5xl md:text-7xl font-cursive text-gray-800 leading-tight">
				{{ $event->wedding->groom_name ?? 'Groom' }} <br> <span class="text-3xl text-gray-400">&</span> <br>
		...
		<div class="relative z-10 max-w-3xl mx-auto px-4 md:px-0">
			...
			<section class="min-h-screen flex flex-col justify-center items-center text-center py-10">
				...
				<h1 class="text-4xl md:text-6xl font-cursive text-gray-800 mb-6" data-aos="zoom-in">
					{{ $event->wedding->groom_name ?? 'Groom' }} <span class="text-pink-400">&</span>
					{{ $event->wedding->bride_name ?? 'Bride' }}
				</h1>
				...
				<div class="bg-white/70 backdrop-blur rounded-3xl p-6 md:p-10 shadow-sm border border-white" data-aos="fade-right">
					<div class="flex flex-col md:flex-row items-center gap-6">
						<div class="w-32 h-32 bg-gray-300 rounded-full overflow-hidden flex-shrink-0 border-4 border-pink-100">
							<img src="https://via.placeholder.com/150" alt="Groom" class="w-full h-full object-cover">
						</div>
						<div class="text-center md:text-left">
							<h3 class="text-2xl font-cursive text-gray-800 mb-2">{{ $event->wedding->groom_name ?? 'The Groom' }}</h3>
							<p class="text-sm text-gray-500 mb-2">Son of {{ $event->wedding->groom_parent ?? 'Mr & Mrs. Groom' }}</p>
				...
				<div class="bg-white/70 backdrop-blur rounded-3xl p-6 md:p-10 shadow-sm border border-white" data-aos="fade-left">
					<div class="flex flex-col md:flex-row-reverse items-center gap-6">
						<div class="w-32 h-32 bg-gray-300 rounded-full overflow-hidden flex-shrink-0 border-4 border-purple-100">
							<img src="https://via.placeholder.com/150" alt="Bride" class="w-full h-full object-cover">
						</div>
						<div class="text-center md:text-right">
							<h3 class="text-2xl font-cursive text-gray-800 mb-2">{{ $event->wedding->bride_name ?? 'The Bride' }}</h3>
							<p class="text-sm text-gray-500 mb-2">Daughter of {{ $event->wedding->bride_parent ?? 'Mr & Mrs. Bride' }}</p>
					...
					<div class="relative border-l-2 border-pink-200 ml-6 md:ml-1/2 space-y-10">
						@foreach ($event->love_stories as $story)
							<div class="relative pl-8 md:pl-12" data-aos="fade-up">
								...
								<span
									class="text-xs font-bold text-pink-400 bg-pink-50 px-2 py-1 rounded mb-2 inline-block">{{ $story->year }}</span>
								<h4 class="text-lg font-bold text-gray-800">{{ $story->title }}</h4>
								<p class="text-sm text-gray-600 mt-1">{{ $story->story }}</p>
							</div>
						@endforeach
					</div>
				</section>
					...
					<div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
						@foreach ($event->gallery as $photo)
							<div class="aspect-square bg-gray-200 rounded-lg overflow-hidden cursor-pointer hover:opacity-90 transition"
								...
								<img src="{{ $photo->photo_url }}" alt="Gallery" class="w-full h-full object-cover">
							</div>
						@endforeach
					</div>
				</section>
				...
				<form action="#" class="space-y-4">
					<div>
						...
						<input type="text"
							...
							placeholder="Your Name" value="{{ $invitation->guest_name ?? '' }}">
					</div>
					<div>
			...
			@if (isset($wishes) && count($wishes) > 0)
				<section class="py-12">
					...
					<div class="space-y-4 overflow-y-auto max-h-96 pr-2 hide-scroll">
			...
			<footer class="text-center py-10 text-gray-400 text-xs">
				<p>&copy; {{ date('Y') }} Undangan Digital.</p>
			</footer>
